...
function HF.GetAfflictionStrength(character, identifier, defaultvalue)
    ...
    if prefab == nil then return defaultvalue end

    local aff = character.CharacterHealth.GetAffliction(prefab)
...
function HF.GetAfflictionStrengthLimb(character, limbtype, identifier, defaultvalue)
    ...
    local aff = character.CharacterHealth.GetAffliction(prefab, limb)
    local res = defaultvalue or 0
    if aff ~= nil then
        res = aff.Strength
    end
...
function HF.SetAfflictionLimb(character, identifier, limbtype, strength, aggressor, prevstrength)
    ...
    local adjustedStrength = strength * character.CharacterHealth.MaxVitality / 100 / (1 - resistance)
    local affliction = prefab.Instantiate(adjustedStrength, aggressor)
